...
export default function SprintsTimeline() {
  ...
  const getStatusBadge = (status: SprintItem['status']) => {
    switch (status) {
      case 'shipped':
        return (
          <span className="inline-flex items-center gap-1.5 bg-emerald-50 border border-emerald-100 text-emerald-700 text-[10px] font-semibold uppercase tracking-wider px-2.5 py-1 rounded-full">
      ...
      case 'active':
        return (
          <span className="inline-flex items-center gap-1.5 bg-amber-50 border border-amber-100 text-amber-700 text-[10px] font-semibold uppercase tracking-wider px-2.5 py-1 rounded-full animate-pulse">
            <Activity size={10} />
            Active
          </span>
        );
      default:
        return (
          <span className="inline-flex items-center gap-1.5 bg-gray-100 border border-gray-200 text-gray-500 text-[10px] font-semibold uppercase tracking-wider px-2.5 py-1 rounded-full">
            Upcoming
          </span>
        );
    }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 font-sans">
      {/* Live stats banner */}
      {live && (
        <div className="bg-white border border-gray-200 rounded-2xl px-5 py-3.5 flex flex-wrap gap-6 text-xs shadow-sm">
          <span className="text-gray-500">
            <span className="text-amber-600 font-semibold text-base font-mono">{live.estimatedPages}</span> est. pages
          </span>
          <span className="text-gray-500">
            <span className="text-blue-600 font-semibold text-base font-mono">{live.content.blogPosts}</span> blog posts
          </span>
          <span className="text-gray-500">
            <span className="text-purple-600 font-semibold text-base font-mono">{live.infrastructure.apiRoutes}</span> API routes
          </span>
          {live.infrastructure.lastBuild && (
            <span className="text-gray-400 font-mono self-center ml-auto">
      ...
      <div>
        ...
        <h1 className="text-2xl font-semibold text-gray-900">Sprint Deliverables Timeline</h1>
      ...
      <div className="relative border-l border-gray-200 ml-3.5 pl-6 space-y-12 py-2">
        {HISTORICAL_SPRINTS.map((sprint, index) => (
          <div key={sprint.id} className="relative group">
            {/* Timeline Bullet Dot */}
            <span className={`absolute -left-[31px] top-1.5 flex items-center justify-center w-4 h-4 rounded-full border bg-white transition-all duration-300 ${
              sprint.status === 'active' 
                ? 'border-amber-500 ring-4 ring-amber-100' 
                : 'border-gray-300 group-hover:border-emerald-500'
            }`}>
              <span className={`w-1.5 h-1.5 rounded-full ${
                sprint.status === 'active' ? 'bg-amber-500' : 'bg-gray-300 group-hover:bg-emerald-500'
              }`} />
            </span>

            {/* Sprint Card */}
            <div className="bg-white border border-gray-200 rounded-2xl p-6 hover:border-gray-300 transition-all duration-300 shadow-sm relative overflow-hidden">
              <div className="absolute top-0 right-0 w-48 h-48 bg-gradient-to-br from-amber-500/[0.01] to-transparent pointer-events-none" />

              {/* Title & Badge Row */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                <div>
                  <h3 className="text-gray-900 text-base font-semibold flex items-center gap-2 flex-wrap">
                    {sprint.name}
                  </h3>
                  <div className="flex items-center gap-1.5 text-gray-500 text-xs font-medium mt-1 font-mono">
                    <Calendar size={12} className="text-gray-400" />
                    {sprint.date}
                    <span className="text-gray-300">•</span>
                    <span className="text-amber-700 text-[10px] uppercase tracking-wider bg-amber-50 border border-amber-100 px-1.5 py-0.5 rounded font-sans font-medium">
                      {sprint.scope}
                    </span>
                  </div>
                </div>
              ...
              <div className="space-y-3">
                <h4 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2 font-mono">
              ...
              {/* Git Commit Hash Marker */}
              <div className="mt-6 pt-4 border-t border-gray-100 flex items-center justify-between text-[10px] text-gray-400 font-mono">

...
export default function BaseLeadForm({
  ...
  const validateField = (name: keyof LeadData, value: string, required?: boolean, type?: string) => {
    if (name === 'mobile') return validateMobile(value, required)
    if (name === 'email') return validateEmail(value, required)
    // For select fields: empty string means nothing selected
    if (type === 'select' && required && !value) return 'Please select an option'
    // For all other required fields (textarea, text), check if empty
  ...
  const handleChange = (name: keyof LeadData, value: string) => {
    ...
    if (touched[name]) {
      ...
      setErrors(prev => ({ ...prev, [name]: validateField(name, value, field?.required, field?.type) }))
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const field of fields) {
      ...
      const err = validateField(field.name, (formData[field.name] as string) || '', field.required, field.type)

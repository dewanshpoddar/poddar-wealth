...
// ── Inline lead-capture card (renders inside a bot bubble) ──────────────────
...
interface PoddarJiChatUIProps {
  ...
  badges: string[]
  statusText: string
  compact?: boolean              // popup = true, section = false
}
...
export default function PoddarJiChatUI({
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden flex flex-col ${compact ? '' : 'h-[520px]'}`}
      ...
      <div className="px-3 py-2 flex gap-1.5 overflow-x-auto scrollbar-hide border-t border-slate-100 flex-shrink-0">
        {chips.map((chip, i) => (
          <button
            key={i}
            onClick={() => chat.sendMessage(chipQueries[i])}

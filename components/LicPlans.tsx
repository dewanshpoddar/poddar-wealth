...
export default function LicPlans({ data }: { data: LicData }) {
  ...
  useEffect(() => {
    if (data?.categories) {
      setActiveTab(Object.keys(data.categories)[0])
    }
  }, [data])
  ...
  const handleGetPlan = (planName: string) => {
    // Dispatch global event to open LeadPopup with specific intent
    ...
    <section className="py-12 bg-gray-50/30 min-h-screen">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-12">
          ...
          >
            {lang === 'en' ? 'Explore LIC Insurance Plans' : 'एलआईसी बीमा योजनाओं का अन्वेषण करें'}
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
              ...
              <div className="flex items-center gap-8 md:border-l border-gray-100 md:pl-8 relative z-10">
                <div className="text-center">
                  ...
                  <div className="flex items-center gap-1.5 text-green-600 font-bold text-13">
                    <Zap size={14} fill="currentColor" /> Verified
                  </div>
                </div>
                <div className="text-center">
                  <div className="text-11 font-bold text-gray-400 uppercase tracking-widest mb-1">MDRT Choice</div>
                  <div className="flex items-center gap-1.5 text-gold font-bold text-13">
          ...
          <AnimatePresence mode="popLayout">
            {activeCategory?.plans.map((plan, idx) => (
              <motion.div
                ...
                className={`bg-white rounded-2xl border transition-all duration-300 flex flex-col h-full overflow-hidden
                  ${expandedPlan === plan.id ? 'border-navy/20 shadow-xl lg:col-span-1' : 'border-gray-100 hover:border-navy/10 hover:shadow-card'}`}
              >
                {/* Card Top */}
                <div className="p-6 pb-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="bg-white border border-gray-50 p-2 rounded-md">
                      ...
                      <div className="text-13 font-bold text-navy">{plan.entryAge}</div>
                    </div>
                    <div className="bg-white border border-gray-50 p-2 rounded-md">
                      <div className="text-10 text-gray-400">S.A. (Cover)</div>
                      <div className="text-13 font-bold text-navy">{plan.sumAssured}</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-auto p-6 pt-0 space-y-3">
                  <button
                    ...
                    className="w-full h-11 bg-gradient-to-r from-navy to-navy-light text-white rounded-xl font-bold text-13 shadow-sm hover:shadow-navy/20 transition-all flex items-center justify-center gap-2 group"
                  >
                          ...
                          <div className="space-y-2">
                            {plan.highlights.map((h, i) => (
                              <div key={i} className="flex items-start gap-2 text-13 text-gray-600">
                                ...
                                <span>{h[lang as keyof typeof h]}</span>
                              </div>
                            ))}
                          </div>
                        </div>
